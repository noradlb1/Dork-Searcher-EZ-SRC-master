    ...
    Friend Class Orange
        ...
        Private Function Connector(ByVal line As String, ByVal proxy As String, ByVal str As Integer) As String
            ...
            Using httpRequest As xNet.HttpRequest = New xNet.HttpRequest()
                ...
                Dim str2 As String = ""
                Try
                    str2 = httpRequest.[Get](String.Concat(New Object() {"http://lemoteur.orange.fr/?kw=", line, "&keap=", str * 7 + 1}), Nothing).ToString()
                    If (str2.Contains("orange.fr")) Then
                        Dim matchCollections As MatchCollection = Regex.Matches(str2, "class=""ellipsisLine"">\n\t\t\t\t(\S{5,500})\t")
                        If (matchCollections.Count <> 0) Then
                            For Each match As System.Text.RegularExpressions.Match In matchCollections
                                SyncLock Me.SyncSave
                                    ...
                                    Me._save.SettingLine(match.Result("$1").Replace("<b>", "").Replace("</b>", "").Replace("amp;", ""))
                                End SyncLock
                            Next
        ...
        Public Sub CreateThread()
            Me.GetSetting()
            Me._message.Message("Get setting Orange.", 0)
            Dim num As Integer = 0
            Do
            ...
            Me._message.Message("Start parsing with Orange.", 0)
        End Sub
