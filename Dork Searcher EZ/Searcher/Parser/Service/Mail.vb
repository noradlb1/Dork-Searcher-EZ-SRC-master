    ...
    Friend Class Mail
        ...
        Private Function Connector(ByVal line As String, ByVal proxy As String, ByVal str As Integer) As String
            ...
            Using httpRequest As xNet.HttpRequest = New xNet.HttpRequest()
                ...
                httpRequest.UserAgent = "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36"
                ...
                Dim str2 As String = ""
                Try
                    str2 = httpRequest.[Get](String.Concat(New Object() {"https://go.mail.ru/search?rf=0010&fm=1&q=", line, "&sf=", Me.page * 10}), Nothing).ToString()
                    If (str2.Contains("""blocked"":true") OrElse Not str2.Contains("mail.ru")) Then
                        ...
                        str1 = "Error"
                    Else
                        Dim matchCollections As MatchCollection = Regex.Matches(str2, """,""url"":""(\S{1,500})"",""orig_url""")
        ...
        Public Sub CreateThread()
            Me.GetSetting()
            Me._message.Message("Get setting Mail.", 0)
            Dim num As Integer = 0
            Do
            ...
            Me._message.Message("Start parsing with Mail.", 0)
        End Sub
