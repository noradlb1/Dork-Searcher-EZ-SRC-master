    ...
    Friend Class ZenSearch
        ...
        Private Function Connector(ByVal line As String, ByVal proxy As String, ByVal str As Integer) As String
            ...
            Using httpRequest As xNet.HttpRequest = New xNet.HttpRequest()
                ...
                Dim str2 As String = ""
                Try
                    str2 = httpRequest.[Get](String.Concat(New Object() {"http://www.zensearch.com/?q=", line, "&t=w&l=en&page=", str + 1}), Nothing).ToString()
                    If (str2.Contains("var ispace_brand = 'zensearch'")) Then
                        Dim matchCollections As MatchCollection = Regex.Matches(str2, "cop=main-title"" target=""_blank"">(\S{5,500})</a></p>")
                        If (matchCollections.Count <> 0) Then
                            For Each match As System.Text.RegularExpressions.Match In matchCollections
                                SyncLock Me.SyncSave
                                    ...
                                    Me._save.SettingLine(match.Result("$1").Replace("amp;", "").Replace("<strong>", "").Replace("</strong>", ""))
                                End SyncLock
                            Next
        ...
        Public Sub CreateThread()
            Me.GetSetting()
            Me._message.Message("Get setting ZenSearch.", 0)
            Dim num As Integer = 0
            Do
            ...
            Me._message.Message("Get setting with ZenSearch.", 0)
        End Sub
