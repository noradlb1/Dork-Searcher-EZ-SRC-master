    ...
    Friend Class Amazon
        ...
        Private Function Connector(ByVal line As String, ByVal proxy As String, ByVal str As Integer) As String
            ...
            Using httpRequest As xNet.HttpRequest = New xNet.HttpRequest()
                ...
                Dim str2 As String = ""
                Try
                    str2 = httpRequest.[Get](String.Concat(New Object() {"http://www.amazon.com/gp/bit/apps/web/SERP/search/ref=bit_abba_def_cr_us?ie=UTF8&page=", str, "&query=", line, "&tagbase=abba"}), Nothing).ToString()
                    If (str2.Contains("amazon.com")) Then
                        Dim matchCollections As MatchCollection = Regex.Matches(str2, "&url=(\S{5,500})"">")
                        If (matchCollections.Count <> 0) Then
                            For Each match As System.Text.RegularExpressions.Match In matchCollections
                                SyncLock Me.SyncSave
                                    ...
                                    Me._save.SettingLine(match.Result("$1").Replace("amp;", "").Replace("%2F", "/").Replace("%3A", ":").Replace("%23", "").Replace("%26", "&").Replace("%3D", "=").Replace("%2E", ".").Replace("%3F", "?").Replace("%2B", "+"))
                                End SyncLock
                            Next
        ...
        Public Sub CreateThread()
            Me.GetSetting()
            Me._message.Message("Get setting Amazon.", 0)
            Dim num As Integer = 0
            Do
            ...
            Me._message.Message("Start parsing with Amazon.", 0)
        End Sub
