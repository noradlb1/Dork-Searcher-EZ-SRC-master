...
Namespace Searcher.Parser.Service.Nigma
    Friend Class Nigma
        ...
        Private ReadOnly _getnigma As Setting = Setting.GetNigma()
        ...
        Private Function Connector(ByVal line As String, ByVal proxy As String, ByVal str As Integer) As String
            ...
            Using httpRequest As xNet.HttpRequest = New xNet.HttpRequest()
                ...
                httpRequest.Cookies.Add("docs_on_page", "50")
                Dim str2 As String = ""
                Try
                    str2 = httpRequest.[Get](String.Concat(New Object() {"http://www.nigma.ru/?startpos=", str * 50, "&s=", line, "&t=web&lang=", Me.lang, "&srt=", Me.sort, Me.search, "&dp=on&sf=1"}), Nothing).ToString()
                    If (Not str2.Contains("Nigma.ru") OrElse str2.Contains("<form action='unban.php'")) Then
                        ...
                        str1 = "Error"
                    Else
                        Dim matchCollections As MatchCollection = Regex.Matches(str2, "<a target=""_blank"" href=""(\S{1,500})"" onclick=")
        ...
        Public Sub CreateThread()
            Me.GetSetting()
            Me._message.Message("Get setting Nigma.", 0)
            Dim num As Integer = 0
            Do
            ...
            Me._message.Message("Start parsing with Nigma.", 0)
        End Sub
        ...
        Private Sub GetSetting()
            ...
            Me.lang = Me._getnigma.Lang
            Me.sort = Me._getnigma.Sort
            Me.search = Me._getnigma.search
        End Sub
    End Class
End Namespace