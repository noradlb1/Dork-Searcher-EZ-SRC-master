    ...
    Friend Class Yahoo
        ...
        Private Function Connector(ByVal line As String, ByVal proxy As String, ByVal str As Integer) As String
            ...
            Using httpRequest As xNet.HttpRequest = New xNet.HttpRequest()
                ...
                httpRequest.UserAgent = "Windows / IE 11: Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko"
                httpRequest.Cookies.Add("sB", "v=1&vm=p&n=100&sh=0&pn=100&rw=new&userset=0")
                Dim str2 As String = ""
                Try
                    str2 = httpRequest.[Get](String.Concat(New Object() {"https://search.yahoo.com/search?p=", line, "&bct=0&b=", str * 10 + 1, "&pz=100"}), Nothing).ToString()
                    If (str2.Contains("Sorry, Unable to process request at this time -- error 999") OrElse Not str2.Contains("yahoo.com")) Then
                        ...
                        str1 = "Error"
                    Else
                        Dim matchCollections As MatchCollection = Regex.Matches(str2, "<a class="" ac-algo fz-l ac-21th lh-24"" href=""(\S{5,500})""")
        ...
        Public Sub CreateThread()
            Me.GetSetting()
            Me._message.Message("Get setting Yahoo.", 0)
            Dim num As Integer = 0
            Do
            ...
            Me._message.Message("Start parsing with Yahoo.", 0)
        End Sub
