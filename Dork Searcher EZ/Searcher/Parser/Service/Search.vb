    ...
    Friend Class Search
        ...
        Private Function Connector(ByVal line As String, ByVal proxy As String, ByVal str As Integer) As String
            ...
            Using httpRequest As xNet.HttpRequest = New xNet.HttpRequest()
                ...
                Dim str2 As String = ""
                Try
                    str2 = httpRequest.[Get](String.Concat(New Object() {"https://www.search.com/web?q=", line, "&qo=homeSearchBox&qsrc=1&ot=organic&page=", str + 1, "&sd=1"}), Nothing).ToString()
                    If (str2.Contains("search.com")) Then
                        Dim matchCollections As MatchCollection = Regex.Matches(str2, "wr_clk\?surl=(\S{5,500})&pingUrl")
                        If (matchCollections.Count <> 0) Then
                            For Each match As System.Text.RegularExpressions.Match In matchCollections
                                SyncLock Me.SyncSave
                                    ...
                                    Me._save.SettingLine(match.Result("$1").Replace("amp;", "").Replace("%2F", "/").Replace("%3A", ":").Replace("%23", "").Replace("%26", "&").Replace("%3D", "=").Replace("%2E", ".").Replace("%3F", "?").Replace("%2B", "+").Replace("!", ""))
                                End SyncLock
                            Next
        ...
        Public Sub CreateThread()
            Me.GetSetting()
            Me._message.Message("Get setting Search.", 0)
            Dim num As Integer = 0
            Do
            ...
            Me._message.Message("Start parsing with Search.", 0)
        End Sub
