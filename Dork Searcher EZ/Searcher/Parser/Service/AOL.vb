    ...
    Friend Class AOL
        ...
        Private Function Connector(ByVal line As String, ByVal proxy As String, ByVal str As Integer) As String
            ...
            Using httpRequest As xNet.HttpRequest = New xNet.HttpRequest()
                ...
                httpRequest.UserAgent = "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36"
                ...
                Dim str2 As String = ""
                Try
                    str2 = httpRequest.[Get](String.Concat(New Object() {"http://search.aol.com/aol/search?q=", line, "&v_t=na&page=", str + 1}), Nothing).ToString()
                    If (str2.Contains("Request denied: source address is sending an excessive volume of requests.") OrElse Not str2.Contains("aol.com")) Then
                        ...
                        str1 = "Error"
                    Else
                        Dim matchCollections As MatchCollection = Regex.Matches(str2, "href=""(\S{5,500})"" property=""f:title""")
        ...
        Public Sub CreateThread()
            Me.GetSetting()
            Me._message.Message("Get setting AOL.", 0)
            Dim num As Integer = 0
            Do
            ...
            Me._message.Message("Start parsing with AOL.", 0)
        End Sub
